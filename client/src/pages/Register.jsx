...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      dispatch(setError(err.response?.data?.error || 'Registration failed. Try a different email.'));
    }
  };

  return (
    <div className="min-h-[calc(100vh-160px)] flex items-center justify-center p-4">
      <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_bottom_left,_var(--tw-gradient-stops))] from-blue-100/20 via-transparent to-emerald-100/20 dark:from-blue-900/10 dark:to-emerald-900/10"></div>
      ...
      <Card className="w-full max-w-md border-white/20 dark:border-slate-800/50 shadow-2xl backdrop-blur-xl bg-white/70 dark:bg-[#020617]/70">
        <CardContent className="p-8">
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Join Armor AI</h1>
            <p className="text-slate-500 dark:text-slate-400 mt-2 text-center text-sm">Start your journey to financial freedom today.</p>
          </div>
          ...
          <div className="mt-8 text-center">
            <p className="text-sm text-slate-500 dark:text-slate-400">
              ...
              <Link to="/login" className="text-blue-600 dark:text-blue-400 font-semibold hover:underline decoration-2 underline-offset-4">
                Sign In
              </Link>
            </p>
          </div>
